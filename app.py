...
voice_option = st.selectbox("🎙️ Choose a Voice", ["Google TTS", "Male Voice", "Custom MP3 Voice"])

if st.button("🔊 Generate Audio (MP3)"):
    with st.spinner("Generating audio..."):
        try:
            if voice_option == "Google TTS":
                tts = gTTS(text)
                with tempfile.NamedTemporaryFile(delete=False, suffix=".mp3") as audiofile:
                    tts.save(audiofile.name)
                    st.success("✅ Google TTS audio ready!")
                    st.audio(audiofile.name)
                    st.download_button("⬇️ Download MP3", open(audiofile.name, "rb").read(), file_name="google_tts_audio.mp3")

            elif voice_option == "Male Voice":
                # Placeholder for Male Voice TTS engine (can be extended later)
                tts = gTTS(text, lang="en", tld="co.uk")
                with tempfile.NamedTemporaryFile(delete=False, suffix=".mp3") as audiofile:
                    tts.save(audiofile.name)
                    st.success("✅ Male Voice audio ready!")
                    st.audio(audiofile.name)
                    st.download_button("⬇️ Download MP3", open(audiofile.name, "rb").read(), file_name="male_voice_audio.mp3")

            elif voice_option == "Custom MP3 Voice":
                custom_voice_path = "/mnt/data/77f6bda4-ecdf-42bf-ada9-0bac39c11b00.mp3"  # Your uploaded voice file
                st.success("✅ Custom MP3 voice ready!")
                st.audio(custom_voice_path)
                st.download_button("⬇️ Download MP3", open(custom_voice_path, "rb").read(), file_name="custom_voice_audio.mp3")
